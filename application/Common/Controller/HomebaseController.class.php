...
class HomebaseController extends AppframeController {
	//验证登录时的返回方式,为'ajax'时以Ajax方式返回
	protected $checkReturnType;
	//商城系统配置
	...
	function _initialize() {
		parent::_initialize();
		//获取返回数据方式的设置
		$this->checkReturnType=I('request.checkReturnType');
		...
		$this->EMallConfig=$this->getEmallConfig();
		
	}

	//加载商城系统配置
	protected function getEmallConfig(){
		$EMallConfig=ApiService::getEMallConfig();
		$this->assign('limitCartGoodsNum',$EMallConfig['SHOPCART_MAXNUM']);
		return $EMallConfig;
	}
	...
	protected function check_login(){
	    $session_user=session('user');
		if(empty($session_user)){
			if($this->checkReturnType=='ajax'){
				$this->ajaxReturn(array('status'=>0,'error'=>'请登录后再进行操作！'));
			}else{
	...
	protected function  check_user(){
		...
		if($user_status==2){
			if($this->checkReturnType=='ajax'){
				$this->ajaxReturn(array('status'=>0,'error'=>'您还没有激活账号，请激活后再使用！'));
			}else{
		...
		if($user_status==0){
			if($this->checkReturnType=='ajax'){
				$this->ajaxReturn(array('status'=>0,'error'=>'此账号已经被禁止使用，请联系管理员！'));
			}else{
	...
	//设置验证登录的数据返回方式
	protected function setCheckReturnType($returnType){
