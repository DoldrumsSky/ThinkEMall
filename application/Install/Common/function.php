 ...
 */
function sp_create_config($config, $authcode,$table_prefix){
    if(is_array($config)){
        //读取配置内容
        ...
        $mainConf=file_get_contents('application/Common/Conf/config.php');
        $mainConf=str_replace('cmf_think_session', $table_prefix.'think_session', $mainConf);
        $mainConf=str_replace('cmf_users', $table_prefix.'users', $mainConf);

        //写入应用配置文件
        if(file_put_contents( 'data/conf/db.php', $conf) && file_put_contents( 'application/Common/Conf/config.php', $mainConf)){
...
function set_session_type(){
    ...
    $mainConf=file_get_contents('application/Common/Conf/config.php');        
    $mainConf=str_replace("SESSION_TYPE'=>''", "SESSION_TYPE'=>'Mysqli'", $mainConf);
    $mainConf=str_replace('cmf_think_session', $table_prefix.'think_session', $mainConf);
    $mainConf=str_replace('cmf_users', $table_prefix.'users', $mainConf);
    if(file_put_contents( 'application/Common/Conf/config.php', $mainConf)){
        sp_show_msg('配置session类型成功');
    }else {
        sp_show_msg('配置session类型失败，您可以手动设置SESSION_TYPE=>Mysqli！', 'error');
    }
    return '';
}
